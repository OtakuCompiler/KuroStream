...
package com.kurostream.extensions.emby
...
@Singleton
class EmbyAdapter @Inject constructor(
    ...
    suspend fun getMovies(limit: Int = 50, startIndex: Int = 0): Result<List<EmbyItem>> =
    ...
    suspend fun getSeries(limit: Int = 50, startIndex: Int = 0): Result<List<EmbyItem>> =
    ...
    fun getImageUrl(itemId: String, type: EmbyImageType = EmbyImageType.Primary, maxWidth: Int = 400): String =
    ...
    suspend fun getServerInfo(): Result<EmbyServerInfo> =
        get("$baseUrl/System/Info").map { json.decodeFromString(it) }
    ...
    private suspend fun get(url: String): Result<String> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            if (!response.isSuccessful) throw Exception("Emby HTTP ${response.code} for $url")
            response.body?.string() ?: throw Exception("Empty response")
        }.onFailure { Timber.e(it, "EmbyAdapter.get($url)") }
    }

    private suspend fun post(url: String, body: String): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            val reqBody = okhttp3.RequestBody.create("application/json".toMediaType(), body)
            val request = Request.Builder()
                .url(url)
                .header("X-Emby-Token", apiKey)
                .post(reqBody)
                .build()
            val response = client.newCall(request).execute()
            if (!response.isSuccessful) throw Exception("Emby POST ${response.code}")
        }.onFailure { Timber.e(it, "EmbyAdapter.post($url)") }
    }

    private fun parseItems(body: String): List<EmbyItem> =
        runCatching {
            json.decodeFromString<EmbyItemsResponse>(body).items
        }.getOrElse { emptyList() }
...
data class EmbyItem(
    ...
    val Studios: List<EmbyStudio> = emptyList(),
) {
...
@Serializable
data class EmbyStudio(val Name: String = "")

@Serializable
data class EmbyServerInfo(
    val ServerName: String = "",
    val Version: String = "",
    val ProductName: String = "Emby Server",
    val Id: String = "",
)

enum class EmbyImageType { Primary, Backdrop, Thumb, Logo, Banner }
