...
class NetworkDashboardViewModel @Inject constructor(
    ...
    private val _networkStats = MutableStateFlow<NetworkStats>(NetworkStats(
        ...
        networkType = com.kurostream.domain.network.NetworkType.UNKNOWN,
  ...
  private fun startMonitoring() {
    viewModelScope.launch {
      try {
        ...
        _networkStats.value = _networkStats.value.copy(networkType = com.kurostream.domain.network.NetworkType.UNKNOWN)
      }
    }
  ...
  fun runSpeedTest() {
    viewModelScope.launch {
      try {
        ...
        _speedTestResult.value = Result.success(result)
      } catch (e: Exception) {
        _speedTestResult.value = Result.error(e)
      }
    }
  }
  ...
  fun runPingTest(host: String = "8.8.8.8") {
    viewModelScope.launch {
      try {
        ...
        _pingTestResult.value = Result.success(result)
      } catch (e: Exception) {
        _pingTestResult.value = Result.error(e)
      }
    }
  }
