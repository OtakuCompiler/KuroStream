...
class RefreshRateSwitcher(
    ...
    private fun findBestMode(contentFps: Float): DisplayMode? {
        if (supportedModes.isEmpty()) return null // Safe fallback: optional value not present
    ...
    private fun getSupportedModes(): List<DisplayMode> {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) return listOf(60.0f) // Default fallback

        val display = displayManager.getDisplay(Display.DEFAULT_DISPLAY) ?: return listOf(60.0f) // Default fallback
