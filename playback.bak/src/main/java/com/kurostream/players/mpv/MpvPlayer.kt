...
import com.kurostream.players.core.PlaybackReliabilityManager
...
class MpvPlayer(private val context: Context) : PlayerInterface {
    ...
    // Audio focus + wake lock (mirrors Media3Player behavior)
    private val audioManager: AudioManager =
        context.getSystemService(Context.AUDIO_SERVICE) as AudioManager
    private val powerManager: PowerManager =
        context.getSystemService(Context.POWER_SERVICE) as PowerManager
    private val wakeLock: PowerManager.WakeLock =
        powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "kurostream:mpv_player")
    @Volatile
    ...
    // PRIORITY 1: Playback Reliability Manager
    private val reliabilityManager = PlaybackReliabilityManager(this)
    ...
    override fun seekTo(positionMs: Long) {
        ...
        reliabilityManager.forceClockResync()
    }
    ...
    override fun loadMedia(mediaItem: MediaItem) {
        ...
        // PRIORITY 1: Start reliability monitoring
        reliabilityManager.startMonitoring()
    }
    ...
    override fun release() {
        ...
        reliabilityManager.stopMonitoring()
        reliabilityManager.release()
        eventLoopJob?.cancel()
        scope.cancel()

        // Release wake lock + audio focus
    ...
    private fun handlePropertyChange(event: MPVEvent) {
        when (event.name) {
            ...
            "speed" -> {
                val newSpeed = (event.data?.toDoubleOrNull() ?: 1.0).toFloat()
                _speed.value = newSpeed
                // PRIORITY 1: Notify reliability manager of speed change
                reliabilityManager.onUserSpeedChange(newSpeed)
            }
            ...
            "display-fps" -> {
                ...
                reliabilityManager.setDisplayRefreshRate(fps.toFloat())
            }
            "estimated-vf-fps" -> {
                val fps = event.data?.toDoubleOrNull() ?: 0.0
                _diagnostics.value = _diagnostics.value.copy(currentFps = fps.toFloat())
                reliabilityManager.setContentFrameRate(fps.toFloat())
            }
            "frame-drop-count" -> {
                // PRIORITY 1: Frame drop detected
                reliabilityManager.onFrameRendered(System.nanoTime(), false)
            }
        }
    }
