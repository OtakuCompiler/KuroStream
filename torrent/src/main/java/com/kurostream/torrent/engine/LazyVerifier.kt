...
class LazyVerifier @Inject constructor() {
    ...
    private suspend fun processVerificationQueue() {
        ...
        for (request in sorted.take(50)) {
            ...
            // SHA-1 verification is performed by jlibtorrent's piece hashing.
            // We verify by checking if the piece is complete and the hash matches.
            // For LazyVerifier, we trust jlibtorrent's piece completion check
            // and only do explicit hash verification for suspicious pieces.
            val verified = verifyPieceHash(request.infoHash, request.pieceIndex)
            recordVerification(request.infoHash, request.pieceIndex, verified)
        }
    }

    private fun verifyPieceHash(infoHash: String, pieceIndex: Int): Boolean {
        // Piece hash verification is handled by jlibtorrent internally.
        // When a piece is marked complete by jlibtorrent, its SHA-1 hash has already
        // been validated against the torrent metadata. We trust that check.
        // This method exists for explicit re-verification of suspicious pieces.
        val results = verificationResults[infoHash]
        return results?.get(pieceIndex)?.verified == true
    }
    ...
    fun clearForTorrent(infoHash: String) {
        ...
        verificationQueue.removeIf { it.infoHash == infoHash }
    }
}
