...
@Singleton
class EmbyLibrarySync @Inject constructor(
    ...
    fun syncLibraries(): Flow<SyncProgress> = flow {
        ...
        val service = authManager.getAuthenticatedService()
            ?: run { emit(SyncProgress.Error("Not authenticated")); return@flow }
        val userId = authManager.getUserId()
            ?: run { emit(SyncProgress.Error("User ID not found")); return@flow }

        try {
            val viewsResponse = service.getViews(userId)
            if (!viewsResponse.isSuccessful) {
                emit(SyncProgress.Error("Failed: ${viewsResponse.code()}"))
                return@flow
            }
            ...
            for (view in views) {
                ...
                if (itemsResponse.isSuccessful) {
                    ...
                    val entities = items.map { mapToEntity(it, view.Id, view.Name) }
    ...
    private fun mapToEntity(item: EmbyLibraryItem, libraryId: String, libraryName: String): LibraryItemEntity {
        ...
        return LibraryItemEntity(
            ...
            durationMs = item.RunTimeTicks?.let { it / 10000 } ?: 0L,
            progressMs = item.UserData?.PlaybackPositionTicks?.let { it / 10000 } ?: 0L,
