...
class KuroAuthServiceTest {
    ...
    @Test
    fun `signInWithPassword returns tokens on success`() = runTest {
        ...
        mockWebServer.enqueue(MockResponse()
            .setResponseCode(200)
            .setBody(kotlinx.serialization.json.Json.encodeToString(KuroAuthTokens.serializer(), tokens))
            .addHeader("Content-Type", "application/json"))

        val response = authApi.signInWithPassword("test-key", body = KuroSignInRequest("user@test.com", "password123"))
    ...
    @Test
    fun `signUp returns tokens on success`() = runTest {
        val tokens = KuroAuthTokens(
        ...
        val response = authApi.signUp("test-key", body = KuroSignUpRequest("new@test.com", "password123"))
    ...
    fun `refreshToken returns new tokens`() = runTest {
        ...
        val response = authApi.refreshToken("test-key", body = KuroRefreshRequest("old-refresh-token"))
    ...
    @Test
    fun `signInWithPassword throws on invalid credentials`() = runTest {
        mockWebServer.enqueue(MockResponse()
            .setResponseCode(400)
        ...
        try {
            authApi.signInWithPassword("test-key", body = KuroSignInRequest("wrong@test.com", "wrong"))
        } catch (e: Exception) {
            exception = e
        }
