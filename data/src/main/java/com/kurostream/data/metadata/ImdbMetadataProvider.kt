...
class ImdbMetadataProvider @Inject constructor(
    ...
    private fun mapToDomain(dto: Title): AnimeMetadata {
        return AnimeMetadata(
            ...
            season = null,
            genres = dto.genres?.genres?.mapNotNull { it.text } ?: emptyList(),
            studios = dto.productionCompany?.edges?.map { it.node?.name }?.filterNotNull() ?: emptyList(),
            score = dto.ratingsSummary?.aggregateRating?.div(10.0),
            scoredBy = dto.ratingsSummary?.voteCount,
            rank = null,
            popularity = dto.ratingsSummary?.popularityScore,
            favorites = null,
            ageRating = dto.certificates?.edges?.firstOrNull()?.node?.rating,
            sourceMaterial = null,
            durationMinutes = dto.runtime?.seconds?.div(60),
